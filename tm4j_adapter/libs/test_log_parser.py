...
def parse_log(file: str) -> list:
    with open(file, 'r') as content:
        return content.read().splitlines()


def parse_test_log(file: str) -> dict:
    test_logs = _split_list(parse_log(file))
    return test_logs
...
def _split_list(input_lines: list) -> dict:
    ...
    return {k: remove_duplicates(v) for k, v in result.items() if k != ' '}
